...
class AbstractEmbeddingSummarizer:
    ...
    def generate_comprehensive_summary(self, knowledge_chunks: List[Dict], query: str) -> str:
        # Use Llama 3.2 for summary generation
        try:
            from app.services.logic.qa_chatbot import get_ollama_response
        except ImportError:
            return "Llama 3.2 model (get_ollama_response) not available."
        ...
        # Concatenate relevant sentences from top chunks
        ...
        if not all_relevant_sentences:
            # Fallback to concatenated abstracts
            all_relevant_sentences = [chunk['text'] for chunk in knowledge_chunks[:5]]
        prompt = f"Summarize the following medical findings for the query '{query}' in clear, concise language for a research audience.\n\n" + "\n".join(all_relevant_sentences)
        summary = get_ollama_response(prompt, "llama3.2")
        return summary if summary else "No summary generated by Llama 3.2."
